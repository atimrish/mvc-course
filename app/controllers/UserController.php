...
class UserController extends InitController
{
    ...
    public function actionAdd()
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST' && !empty($_POST['btn_add_user'])) {
            ...
            if (!empty($user)) {
                ...
                if (empty($user['password'])) {
                    $error_msg['password'] = '1';
                }
                if (empty($user['password_confirm'])) {
                    $error_msg['password_confirm'] = '1';
                }
                if ($user['password'] !== $user['password_confirm']) {
                    $error_msg['incorrect_password'] = '1';
                }
                if (!isset($user['is_admin'])) {
                    $error_msg['is_admin'] = '1';
                }
                if (empty($error_msg)) {
                    $users_model = new UsersModel();
                    $result = $users_model->addNewUser($user['name'], $user['login'], $user['password']);
    ...
    public function actionUpdate() {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST' && !empty($_POST['btn_update_user'])) {
            $user = !empty($_POST['user']) ? $_POST['user'] : null;
            ...
            if (!empty($user)) {
                ...
                if (empty($error_msg)) {
                    $result = (new UsersModel())->updateUser($user);
    ...
    {
        $result = (new UsersModel())->deleteUser();
        if (!empty($result)) {
            $this->redirect('/user/users');
        }
    }
